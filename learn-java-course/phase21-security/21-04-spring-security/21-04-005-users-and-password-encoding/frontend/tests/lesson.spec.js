...
const jarName = 'spring-security-lesson005-1.0-SNAPSHOT.jar';
let before, secured, beforeUi, securedUi;

async function startBackend(jar, port = 0) {
  const state = { logs: '', port: null, process: spawn(java, ['-jar', jar, `--server.port=${port}`, '--spring.security.user.password=lesson005-default-test',
    '--lesson.users.member-password=lesson005-test-only', '--lesson.users.support-password=lesson005-support-test',
...
test.beforeAll(async () => {
  ...
  before = await startBackend(resolve(frontend, '.e2e-work/before-users/21-04-005-users-and-password-encoding/target', jarName));
...
test.afterAll(async () => {
  ...
  writeFileSync(resolve(frontend, '.e2e-work/browser-summary.json'), JSON.stringify({
    defaultUserPublicCalls: before ? calls(before, 'PUBLIC_INFO_HANDLER_REACHED') : null,
...
test('默认用户对照：新增member尚不能认证，显式用户版可以', async ({ page }) => {
  await page.goto(beforeUi.url);
  await page.locator('#identity-password').fill('lesson005-test-only');
...
test('三种身份、错误密码与清空：凭据不进入观察区或存储，下一次匿名仍401', async ({ page }, info) => {
  await page.goto(securedUi.url);
  for (const [name, password] of [['member', 'lesson005-test-only'], ['support', 'lesson005-support-test'], ['admin', 'lesson005-admin-test']]) {
