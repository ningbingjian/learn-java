...
const jarName = 'spring-security-lesson007-1.0-SNAPSHOT.jar';
let before, secured, beforeUi, securedUi;

async function startBackend(jar, port = 0) {
  const state = { logs: '', port: null, process: spawn(java, ['-jar', jar, `--server.port=${port}`, '--spring.security.user.password=lesson007-default-test',
    '--lesson.users.member-password=lesson007-test-only', '--lesson.users.support-password=lesson007-support-test',
...
test.beforeAll(async () => {
  ...
  before = await startBackend(resolve(frontend, '.e2e-work/before-login/21-04-007-form-login-session/target', jarName));
...
test('默认登录响应：浏览器禁止跳转后结果不明，但会话可能已经建立', async ({ page }) => {
  ...
  await page.locator('#login-password').fill('lesson007-test-only');
...
test('三种身份、错误密码与清空：凭据不进入观察区或存储，下一次匿名仍401', async ({ page }, info) => {
  await page.goto(securedUi.url);
  for (const [name, password] of [['member', 'lesson007-test-only'], ['support', 'lesson007-support-test'], ['admin', 'lesson007-admin-test']]) {
...
test('真实表单登录：编码、会话轮换、令牌刷新与匿名对照', async ({ page, context }, info) => {
  ...
  await page.getByRole('button', { name: '访问受保护接口', exact: true }).click();
  await expect(page.locator('#status-code')).toHaveText('HTTP 401');
  await expect(page.locator('body')).not.toContainText('lesson007-test-only');
  expect(await page.evaluate(() => [localStorage.length, sessionStorage.length])).toEqual([0, 0]);
