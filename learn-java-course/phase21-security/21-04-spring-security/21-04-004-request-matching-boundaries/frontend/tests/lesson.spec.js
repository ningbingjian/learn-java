...
const jarName = 'spring-security-lesson004-1.0-SNAPSHOT.jar';
...
test.beforeAll(async () => {
  ...
  before = await startBackend(resolve(frontend, '.e2e-work/wrong-order/21-04-004-request-matching-boundaries/target', jarName));
...
test.afterAll(async () => {
  ...
  writeFileSync(resolve(frontend, '.e2e-work/browser-summary.json'), JSON.stringify({
    wrongOrderPublicCalls: before ? calls(before, 'PUBLIC_INFO_HANDLER_REACHED') : null,
...
test('宽范围拒绝先命中：错序公开401，完成版恢复200', async ({ page }) => {
  await page.goto(beforeUi.url);
  await page.getByRole('button', { name: '访问公开信息', exact: true }).click();
