...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
        properties = {"lesson.users.member-password=lesson009-test-only",
                "lesson.users.support-password=lesson009-support-test",
...
class RequestBoundaryHttpTests {
    ...
    void validPasswordReturnsBusinessJson() throws Exception {
        var response = get("/hello", "application/json", basic("member", "lesson009-test-only"));
        assertThat(response.statusCode()).isEqualTo(200);
    ...
    void nextRequestWithoutCredentialsIsStillRejected() throws Exception {
        assertThat(get("/hello", "application/json", basic("member", "lesson009-test-only"))
    ...
    void missingRouteIsProtectedBeforeMvcResolvesIt() throws Exception {
        ...
        assertThat(get("/missing", "application/json", basic("member", "lesson009-test-only"))
    ...
    void authenticatedSimilarAndUnlistedPathsAreDeniedInsteadOfReachingMvc() throws Exception {
        for (String path : new String[]{"/public/info-extra", "/public/info/private", "/public/other",
            ...
            var response = get(path, "application/json", basic("member", "lesson009-test-only"));
    ...
    void optionsDoesNotInheritThePublicGetRule() throws Exception {
        ...
        assertThat(request("OPTIONS", "/public/info", basic("member", "lesson009-test-only"))
                .statusCode()).isEqualTo(403);
    }
    ...
    void headDoesNotInheritGetAuthorizationEvenThoughMvcSupportsHead() throws Exception {
        ...
        assertThat(request("HEAD", "/public/info", basic("member", "lesson009-test-only"))
                .statusCode()).isEqualTo(403);
    }
    ...
    void loginProcessingWithCsrfStillAuthenticatesAndCookieCanAccessListedHello() throws Exception {
        ...
        String form = "username=member&password=lesson009-test-only&_csrf="
    ...
    void threeIdentitiesAuthenticateButAdminDoesNotBypassDefaultDeny() throws Exception {
        for (String[] user : new String[][]{{"member", "lesson009-test-only"},
                {"support", "lesson009-support-test"}, {"admin", "lesson009-admin-test"}}) {
    ...
    void bootDefaultUserIsNoLongerAvailable() throws Exception {
        assertAuthError(get("/hello", "application/json", basic("user", "lesson009-test-only")));
    }
