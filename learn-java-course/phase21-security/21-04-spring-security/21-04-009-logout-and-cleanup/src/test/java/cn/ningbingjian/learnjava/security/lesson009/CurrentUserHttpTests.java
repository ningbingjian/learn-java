...
class CurrentUserHttpTests {
    ...
    void threeUsersAreRestoredByCookieAndOnlyDisplayFieldsAreReturned() throws Exception {
        for (String[] row : new String[][]{{"member", "lesson009-test-only", "ROLE_MEMBER"},
                {"support", "support+&= 汉#test", "ROLE_SUPPORT"}, {"admin", "lesson009-admin-test", "ROLE_ADMIN"}}) {
    ...
    void explicitBasicIdentifiesOnlyThatRequestWithoutTransferringItsIdentity() throws Exception {
        String credentials = java.util.Base64.getEncoder().encodeToString("member:lesson009-test-only".getBytes(StandardCharsets.UTF_8));
