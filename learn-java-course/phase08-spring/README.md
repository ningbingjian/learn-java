...
| 编号 | 模块 | 学习范围 |
| --- | --- | --- |
| 08-01 | Spring Core / IoC | 从依赖注入入门到容器运行机制、完整核心源码系列与扩展开发 |
...
先确认各模块的职责与依赖，再展开 08-01 Spring Core / IoC 的完整模块大纲，之后按课编写文档与配套源码。每个模块保留入门、进阶、原理、源码与工程实践的完整路径；课程数量由内容和依赖决定。
