...
## 13. 本课总结：把 XML 装配与定义继承串起来

### 13.1 本课解决了什么问题

本课从一个通知器开始，解决了两个相互关联的问题：如何用 XML 描述对象的创建与依赖装配，以及多个对象的配置重复时如何复用公共规则。随后通过集合合并、元数据观察和源码阅读，解释这些规则在容器中怎样生效。

学完后，你应当能独立写出一组有依赖、有初始化要求的 XML 配置，并说明每项配置在哪个阶段起作用；遇到错误时，也能判断应该检查定义、依赖解析还是对象初始化。

### 13.2 把核心原理连成一条主线

**第一步，读取配置，登记对象定义。** XML 读取器把 bean 名称、创建入口、参数、属性和父定义名称等信息保存为 BeanDefinition。此时保存的是创建规则；例如 `ref` 先表示要查找哪个对象，不代表读取到它时就已经拿到了对象。

**第二步，根据父子关系形成有效定义。** 存在 parent 时，容器以父配置为基础，纳入子配置。父模板复用的是配置，不需要先创建父通知器。子集合默认替换同名父集合；开启 merge 后，List 按父后子追加，Map 的同名键由子值覆盖。这些计算发生在定义层面。

**第三步，按有效定义创建并装配对象。** 容器准备构造器或工厂方法需要的参数，将引用解析成真实对象，再取得当前实例。随后解析并填充 setter 属性，执行初始化回调，使对象达到业务可用状态。引用、类型、可写属性和初始化约束，都需要在相应阶段得到满足。

**第四步，使用对象并由所属容器管理清理。** 本例通过容器取得通知器、调用发送方法，并在所属上下文关闭时执行单例的销毁回调。子容器能够通过父级查找使用父对象，但持有引用不会让它接管父对象的销毁。

这是一条按职责梳理的主线。实际创建一个对象的依赖时，可能继续触发另一个对象的创建；不能理解成容器必须先统一合并所有定义，再统一创建所有实例。

### 13.3 需要记住的结论与边界

| 本课知识 | 核心结论 | 使用时要注意 |
| --- | --- | --- |
| value 与 ref | 前者提供值，后者描述命名对象引用 | 文本恰好等于 Bean 名，不会自动成为对象引用 |
| 构造参数与 property | 构造参数参与取得实例，setter 属性在构造后填充 | 依赖完整 setter 配置的检查，应放到初始化阶段 |
| 父子定义 | 合并创建和装配规则，再创建各自的子 Bean | 不要求 Java extends；目标类仍须能接收继承来的配置 |
| abstract 模板 | 定义存在，但不能直接作为业务实例请求 | Spring 定义标志与 Java 抽象类是不同概念 |
| 集合合并 | 子集合显式开启 merge，按集合规则计算结果 | List 不负责业务去重；未开启合并时同名子集合默认替换 |
| 原始定义与合并定义 | 前者记录子配置，后者表达考虑父配置后的有效规则 | 查询到合并定义，不等于业务对象已创建 |
| 工厂与 Java 配置 | 可以改变创建入口或配置表达方式 | 仍要保留有效属性、依赖、作用域及生命周期语义 |
| 父子容器 | 子级可以向父级查找对象 | 查找可见性不等于本地拥有定义，也不等于拥有销毁责任 |

定义继承中的开关还要按具体字段理解。例如本课 XML 入口会把未填写的 lazy-init 按文档默认值解析，再参与合并；“XML 没写”并不总是“定义中没有值”。

### 13.4 如何用于实际配置与排错

多个 Bean 确实共享装配规则时，可以提取父模板；遇到集合差异，先判断业务需要整体替换还是保留公共项后追加，再选择是否开启 merge。迁移成 Java 配置时，比较的是最终对象行为和管理语义，而不仅是 XML 是否删掉。

排错时也沿着本课主线逐层判断：资源是否加载、定义是否登记、父子配置是否得到预期结果、引用和类型能否解析、属性是否可写、初始化约束是否满足。这样才能解释“XML 格式正确，但容器启动仍失败”这一类问题。

最后用自己的话回答三个问题，检查是否已经掌握本课：

1. 为什么抽象模板不需要实例，邮件与短信仍能得到公共配置？
2. 为什么邮件列表的两项内容能在对象创建前，就从合并定义中观察到？
3. 为什么工厂已经返回 Notifier，容器仍需要继续填充属性和执行初始化？

可以用前面的第7、9、10节核对解释。回答时把配置、处理阶段与结果连起来，而不只复述术语。
