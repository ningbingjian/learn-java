...
下一课：[08-01-005 把第一个应用变成可维护的测试](00-模块学习大纲.md#lesson-005)（大纲已列出，正文待编写）。
...
每课的文档、子模块 POM 与 src 放在同一目录；本目录的父 POM 统一管理依赖和构建插件。第001—002课使用普通 Java，只有测试依赖 JUnit；第003课引入 Spring Framework 7.0.9，通过 Maven 运行并附加同版本源码；第004课继续学习 Java 配置与对象装配；聚合构建共运行 31 个测试。
