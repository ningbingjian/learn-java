...
第001—008课首页按连续跟写组织：先完成当前需求，再修改指定文件、运行验证，并联系原理与源码。第001—005课逐步沿用通知接口和业务关系，第006课进入定义模型，再接第007课扫描与第008课 XML。各课的 `02-机制与边界参考.md` 保留完整样例和进阶边界，适合完成跟写后查阅。

跟写的 `practice` 文件由学习者按正文创建，仍在原 Maven 子模块中构建。下列测试数量统计预置源码，不包含本地跟写时新增的测试。
